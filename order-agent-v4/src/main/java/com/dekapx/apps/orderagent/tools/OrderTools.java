...
public class OrderTools {
    ...
    @Tool(description = "Semantically search the product catalog using a natural-language description " +
            "of what the customer wants (not a keyword/SKU lookup). Use this when the user describes a " +
            "need or use case (e.g. 'something for wrist pain while typing', 'a good gift under $100 for " +
            "a music lover') rather than naming an exact product.")
    public String semanticProductSearch(
            @ToolParam(description = "Natural language description of the desired product") String query,
            @ToolParam(description = "Max number of results to return, default 5") Integer topK) {
        log.info("[tool] semanticProductSearch({})", query);
        int k = (topK == null || topK <= 0) ? 5 : topK;
        List<ProductSearchResult> results = productSearchService.search(query, k);
        if (results.isEmpty()) {
            return "No matching products were found for: " + query;
        }
        return results.stream()
                .map(r -> "%s - %s ($%s, %s in stock) [match score %.2f]".formatted(
                        r.sku(), r.name(), r.price(), r.stockQuantity(), r.similarityScore()))
                .reduce((a, b) -> a + "\n" + b)
                .orElse("No matching products were found.");
    }
}
